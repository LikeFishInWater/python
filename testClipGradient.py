...
# train_op = my_opt.apply_gradients(zip(clipped_gradients, params))

gradients = my_opt.compute_gradients(loss, params)
clipped_grads_and_vars = [(tf.clip_by_norm(grad, 10),var) for grad, var in gradients]
train_op = my_opt.apply_gradients(clipped_grads_and_vars)
...
for i in range(10000):
    ...
    sess.run(train_op, feed_dict={x: random_x, y: random_y})
